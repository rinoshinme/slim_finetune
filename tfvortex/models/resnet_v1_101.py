...
class ResNetV1_101(object):
    ...
    def __init__(self, options):
        ...
        if options.PHASE == 'train':
            if options.TRAIN_LAYERS == 'default':
                self.train_layers = self.DEFAULT_TRAIN_LAYERS
            else:
                self.train_layers = options.TRAIN_LAYERS

            # Load train and validation
            with arg_scope(resnet_v1.resnet_arg_scope(activation_fn=activation_function,
    ...
    def get_init_weights_path(self, weights_root):
        return os.path.join(weights_root, 'resnet_v1_101.ckpt')
